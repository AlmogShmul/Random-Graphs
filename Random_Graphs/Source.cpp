...
void buildTheArray(float *array, float threshold )
{
	...
	for (j = 1; j <= 10; j++)
	{
		j < 6 ? array[j - 1] = Random(0, threshold):
			array[j - 1] = Random(threshold, (threshold +  0.008));
	}
	sort(array, array + 10);
}
...
float testIsolated( int v, float p, int runs){
	...
	for (int j = 0; j < runs; j++)
	{
		...
		sum +=is_isolated(g);
		cout << "finished run: " << j << " in testIsolated out of " << runs << " tests." << endl;
	}
...
float testConnective(int v, float p, int runs){
	...
	for (int j = 0; j < runs; j++)
	{
		Graph g = build_random_graph(v, p);
		sum +=connectivity(g);
		cout << "finished run: " << j << " in testConnective out of " << runs << " tests." << endl;
	}
	return sum / float(runs);
...
int main()
{
	...
	createCSVTest("table1.csv",testConnective,v,num_of_grps,num_of_probs,attr1);
	createCSVTest("table2.csv",testDiameter,v,num_of_grps,num_of_probs,attr2);
	createCSVTest("table3.csv",testIsolated,v,num_of_grps,num_of_probs,attr3);
